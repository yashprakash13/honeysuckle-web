...
RATING_MAPPING = {"O": 5, "E": 4, "A": 3, "P": 2, "D": 1, "T": 0}
...
class AllStoriesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        all_stories_queryset = StoryRating.objects.filter(
            created_by=request.user
        ).values_list("story_id", "rating")
        ...
        story_filter = AllStoriesFilter(
            request.GET, queryset=Story.objects.filter(story_id__in=idlist)
        )
        ...
        all_stories = [
            (story, id_rating_dict[story["story_id"]]) for story in all_stories
        ]
