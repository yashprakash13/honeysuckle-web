...
class SearchEngine(Indices):
    ...
    def _continuous_append(self, rows_appended, dfs):
        ...
        while rows_appended <= NUM_MAX_RESULT_FETCH:
            for df in dfs:
                dftemp = dftemp.append(df.iloc[rows_appended:rows_appended+NUM_SUBSEQUENT_ROW_APPENDS], ignore_index=True)
    ...
    def _order_rows_from_df(self, df, cat_property, cat_category, num_rows=None, ordered=True):
        ...
        """
        if not num_rows:
            return df.iloc[(pd.Categorical(df[cat_property], 
                                                 categories=cat_category, 
                                                 ordered=ordered).argsort())]
        else:
            return df.iloc[(pd.Categorical(df[cat_property], 
                                                 categories=cat_category, 
                                                 ordered=ordered).argsort())].head(num_rows)
    ...
    def _merge_res_dfs(self, dfdict):
        ...
        if dfdict['div_0']  and dfdict['div_1'] and dfdict['div_2'] and dfdict['div_3']:
            ...
            rows_appended += NUM_SUBSEQUENT_ROW_APPENDS
            # do CA
            df_to_return = df_to_return.append(self._continuous_append(rows_appended, 
                                                                       [dftemp2, 
                                                                        dftemp, 
                                                                        dftemp3, 
                                                                        dftemp4]))
    ...
    def _title_search(self):
        ...
        res_r = process.extract(' '.join([word for word in self.query.split()if word.lower() not in (stop)]), 
                              df['title_without_stopwords'], 
                              scorer=fuzz.ratio, 
                              limit=STANDARD_LENGTHS_TO_RETURN, score_cutoff=STANDARD_SCORE_CUTOFF)
        res_WR = process.extract(' '.join([word for word in self.query.split()if word.lower() not in (stop)]), 
                              df['title_without_stopwords'], 
                              scorer=fuzz.WRatio, 
                              limit=STANDARD_LENGTHS_TO_RETURN, score_cutoff=STANDARD_SCORE_CUTOFF)
        ...
        if res_r[0][1] < 100:  
            return story_ids_to_return, df.loc[df['story_id'].isin(story_ids_to_return[:1])]['title'].item()
        else:
    ...
    def _author_search(self):
        ...
        res_r = process.extract(self.author_search_word, all_authors, scorer=fuzz.ratio, 
                                limit=STANDARD_LENGTHS_TO_RETURN, 
                                score_cutoff=STANDARD_SCORE_CUTOFF)
        res_WR = process.extract(self.author_search_word, all_authors, scorer=fuzz.WRatio, 
                                 limit=STANDARD_LENGTHS_TO_RETURN, 
                                 score_cutoff=STANDARD_SCORE_CUTOFF)
    ...
    def _set_df_to_use(self):
        ...
        if self.pairs_selected:
            ...
            self.df_to_use = self.df_to_use.append(self.class_indices.df.
                                                   loc[self.class_indices.
                                                       df['story_id'].isin(self.class_indices.
                                                                       data_una[self.pairs_selected[0]])])
